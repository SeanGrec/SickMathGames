...
if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
  $query = "SELECT users.id, highscores.react_score FROM users INNER JOIN highscores ON
...
users.id = highscores.user_id ORDER BY highscores.react_score ASC LIMIT 5;";
$result = mysqli_query($connection, $query);
...
<head>
  ...
  <link rel="stylesheet" href="./styles/react-gfx.css">
...
<body>
  ...
  <div class="game-viewer">
    <div class="game-container">
      ...
      <div id="player-highscore">Highscore: <?php echo isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true ? $_SESSION["react_score"] . 'ms' : '?'; ?></div>
    </div>
    <div class="games">
  ...
  <div class="game-top-5">
    ...
    <div class="top-5-scores">
      <?php 
        for ($i = 0; $i < count($react_rows); $i++) {
          echo('<div class="placement-icon"><img src="./images/' . strval($i+1) . '-icon.png"></div><div class="placement-info"><p class="player">' . $react_rows[$i]["username"] . '</p>' . '<p class="score">' . $react_rows[$i]["react_score"] . 'ms</p></div>');
        }
      ?>
    </div>
  </div>
